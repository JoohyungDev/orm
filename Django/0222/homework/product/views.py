...
def product_index(request):
    ...
    sorted_product = sorted(product_db, key=lambda x: x["sale_count"], reverse=True)
...
# 상품 ID(pk)를 인자로 받아 db에서 해당하는 상품 정보를 찾음
def product_detail(request, pk):
    # 찾은 상품 정보를 context에 저장
    context = {"product": product_db[pk - 1]}
    # render함수를 통해 이 딕셔너리를 detail.html 템플릿 파일로 전달
    # render 함수의 반환값은 HTTP 응답 객체로, 사용자에게 전송되어 웹 페이지를 보여줌
