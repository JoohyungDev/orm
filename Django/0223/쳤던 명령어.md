...
from django.shortcuts import render
# 템플릿을 렌더링하여 그 결과를 문자열로 반환하는 함수
# 주로 이메일 메시지, JSON과 같이 HTTP 응답 외부에서 HTML을 생성할 때 사용
from django.template.loader import render_to_string
# HTTP 응답을 반환하는 데에 사용하는 클래스
...
def blog_test(request):
    ...
    # 템플릿 파일의 경로, 템플릿에 전달할 데이터를 인자로 받음
    ...
    # safe=True가 기본 값이며, Ture일 때 data는 딕셔너리 타입이다.
...
################################
# blog > admin.py
# admin 사이트에서 데이터를 생성, 조회, 수정, 삭제하는 등의 작업을 웹 인터페이스로 수행 가능

from django.contrib import admin
from .models import Post

# Post 모델의 데이터를 admin 사이트에서 보고, 새로운 Post 데이터를 생성하고, 기존의 Post 데이터를 수정하거나 삭제할 수 있습니다.
...
# CRUD 연습을 위한 shell 사용
# ORM : 파이썬 문법을 통해 DB를 조작할 수 있는 편리한 기술(모델 클래스와 DB의 테이블을 자동으로 연관지어 주는 기술)
...
# create는 save가 필요 없지만 수정까지 하고 save안하는 경우가 있어 무조건 하는것을 추천함
>>> q.save()
>>> Post.objects.all()
